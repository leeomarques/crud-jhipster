...
export class PedidoService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/pedidos', 'minicurso');
  ...
  protected convertDateFromClient(pedido: IPedido): IPedido {
    return Object.assign({}, pedido, {
      dtEntrega: pedido.dtEntrega?.isValid() ? pedido.dtEntrega.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dtEntrega = res.body.dtEntrega ? dayjs(res.body.dtEntrega) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((pedido: IPedido) => {
        pedido.dtEntrega = pedido.dtEntrega ? dayjs(pedido.dtEntrega) : undefined;
      });
    }
    return res;
  }
